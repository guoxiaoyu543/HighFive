...
///
/// Get the parent of a path. For example for ``path =
/// "/path/to/dataset"`` this function returns ``"/path/to"``.
///
...
/// created. For example for ``path = "/path/to/dataset"``, this function will
/// create the groups
/// ``"/path"`` and ``"/path/to"``.
///
...
///
inline void createGroupsToDataSet(HighFive::File& file,
...
size_t size(const HighFive::File& file, const std::string& path);
