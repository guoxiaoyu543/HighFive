...
void write(HighFive::DataSet& dataset,
           const Eigen::Matrix<C, Rows, Cols, Options>& data) {
    if (data.IsRowMajor) {
...
struct dump_impl {
    ...
    static HighFive::DataSet run(HighFive::File& file,
        ...
        HighFive::DataSet dataset = file.createDataSet<C>(path,
        ...
        for (std::size_t i = 0; i < shape.size(); ++i) {
            if (shape[i] != dims[i]) {
                throw detail::error(file, path,
                                    "HighFive::dump: Inconsistent dimensions");
            }
        }
...
struct load_impl<Eigen::Matrix<T, Rows, Cols, Options>> {
    static auto run(const HighFive::File& file, const std::string& path) {
        return detail::eigen::load_impl<Eigen::Matrix<T, Rows, Cols, Options>>::run(file,
                                                                                    path);
    }
};
}  // namespace detail
...
inline HighFive::DataSet dump(HighFive::File& file,
                              const std::string& path,
                              const Eigen::Matrix<T, Rows, Cols, Options>& data,
                              HighFive::Mode mode) {
    if ((mode == HighFive::Mode::Create) or
        (mode == HighFive::Mode::Overwrite and !file.exist(path))) {
        return detail::eigen::dump_impl<Eigen::Matrix<T, Rows, Cols, Options>>::run(
            file, path, data);
    }

    return detail::eigen::overwrite_impl<Eigen::Matrix<T, Rows, Cols, Options>>::run(
        file, path, data);
}

template <class T, int Rows, int Cols, int Options>
inline HighFive::DataSet overwrite(HighFive::File& file,
                                   const std::string& path,
                                   const Eigen::Matrix<T, Rows, Cols, Options>& data) {
    return detail::eigen::overwrite_impl<Eigen::Matrix<T, Rows, Cols, Options>>::run(
        file, path, data);
