...
struct dump_impl {
    ...
    run(HighFive::File& file, const std::string& path, const std::vector<C>& data) {
        createGroupsToDataSet(file, path);

        HighFive::DataSet dataset = file.createDataSet<T>(
    ...
    run(HighFive::File& file, const std::string& path, const C& data) {
        ...
        if (dims.size() > 1) {
            throw detail::error(file, path,
                                "HighFive::dump: Can only overwrite 1-d vectors");
        }
